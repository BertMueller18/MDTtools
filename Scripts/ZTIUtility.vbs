	...
	Function CreateEvent(iEventID, iType, sMessage, arrParms)
		...
		If oEnvironment.Item("EventService") <> "" then
			...
			If oService is Nothing then
				oLogging.CreateEntry "Unable to create RESTAPI WebService class", LogTypeWarning
				Exit Function
			End if
			...
			Set wsService = Nothing
                         ...
                         If wsService is Nothing then
                                 oLogging.CreateEntry "Unable to create MDT EVENT WebService class", LogTypeWarning
                                 Exit Function
                         End if
			...
			sLine = sLine & "&dartIP=" & oEnvironment.Item("DartIP001") & "&dartPort=" & oEnvironment.Item("DartPort001") & "&dartTicket=" & oEnvironment.Item("DartTicket") & "&vmHost=" & oEnvironment.Item("VMHost") & "&vmName=" & oEnvironment.Item("VMName")
			 ...
			 ' Add additional parameters specified in MonitorProperties
                         ' These parameters are all for REST API interface in sysprod
                         ' Remember: the first four parameters needs =, all the orthers after data ^ icnluded all vars in cs.ini
                         Dim vData
						 Dim objXmlHttpMain , URL, confIP, connectIP, Progress, Logs
						 Dim Pk
                        ' vData  = "so=" & oEnvironment.Item("So") & "&rack=" & oEnvironment.Item("Rack") & "&shelf=" & oEnvironment.Item("Shelf") & "&data=" & oEnvironment.Item("Data") & "|status^" & oEnvironment.Item("_SMSTSCurrentActionName") & "|progress^" & Round(sCurrentStep / sTotalSteps * 100) & "|model^" & oEnvironment.Item("Model") & "|serial^" & oEnvironment.Item("SerialNumber")
                       '  vData = vData & "|ram^" &  oEnvironment.Item("Memory") & "|cpu^" & oEnvironment.Item("cpu") & "|hostname^" & oEnvironment.Item("ComputerName") & "|installationIP^" & oEnvironment.Item("IpAddress001")
                       '  vData = vData & "|firmware^" & oEnvironment.Item("firmware") & "|disksCount^" & oEnvironment.Item("disksCount") & "|NetIntCount^" & oEnvironment.Item("NetIntCount") & "|image^" & oEnvironment.Item("_SMSTSType")
							Pk = "[" & oEnvironment.Item("So") & "][" & oEnvironment.Item("Rack") & "_" & oEnvironment.Item("Shelf") & "]"
							Logs = "<a href='http://10.0.205.203/Logs/" & oEnvironment.Item("Rack") & "_" & oEnvironment.Item("Shelf") & "/BDD.log' target = '_blank'>Live Logs</a>"
							confIP = oEnvironment.Item("Data")
							confIP = LCase(confIP)
							confIP = """" &  Replace(confIP," : ", """:""") & ""","
							Progress = Int(  oEnvironment.Item("_SMSTSNextInstructionPointer") / oEnvironment.Item("_SMSTSInstructionTableSize") * 100 )
							
							connectIP = "<a href='http://" & oEnvironment.Item("IpAddress001") & ":5800' target='_blank'>vnc://" & oEnvironment.Item("IpAddress001") & "</a>"
							If Progress >= 60 Then
							connectIP = "rdp://" & oEnvironment.Item("IpAddress001")
							End If
							Dim Phase
							Phase = ""
							vData = "{"
							vData = vData & """notifid"":""" & Pk & ""","
							' Workaround for monitoring bug on uefi
                                                         If ((sCurrentStep = 0) Or (sTotalSteps = 0)) Then
                                                                Progress = 100
								Phase = "MDT end. Next step is reboot to deploy final config. (End Task)"
							 End If
							vData = vData & """status"":""<b>" & oEnvironment.Item("_SMSTSCurrentActionName") & "<br /> " & sMessage & "<br />" & Phase & "</b><br />"& Logs & ""","
							vData = vData & """progress"":""" & Progress & ""","
							vData = vData & """model"":""" & oEnvironment.Item("Model") & ""","
							vData = vData & """serial"":""" & oEnvironment.Item("SerialNumber") & ""","
							vData = vData & """ram"":""" & oEnvironment.Item("Memory") & ""","
							vData = vData & """cpu"":""" & oEnvironment.Item("CPU") & ""","
							vData = vData & """hostname"":""" & oEnvironment.Item("ComputerName") & ""","
							vData = vData & confIP
							vData = vData & """installationip"":""" & connectIP & ""","
							vData = vData & """firmware"":""" & oEnvironment.Item("Make") & ""","
							vData = vData & """diskscount"":""" & oEnvironment.Item("disksCount") & ""","
							vData = vData & """netintcount"":""" & oEnvironment.Item("NetIntCount") & ""","
							vData = vData & """os"":""" & oEnvironment.Item("OSCurrentVersion") & " Microsoft Windows "","
							vData = vData & """image"":""" & oEnvironment.Item("TaskSequenceID") & """"
							vData = vData & "}"
						 'msgBox(vData)
			...
			'wsService.WebService = oEnvironment.Item("WebSysprod") & "/api/provisioningnotifications/" & Pk & "/" & vData
						'wsService.Parameters = "json=" & vData
                        'wsService.Method = "POST"
                        'wsService.Quiet = true
                        'Set wsResult = wsService.Query
